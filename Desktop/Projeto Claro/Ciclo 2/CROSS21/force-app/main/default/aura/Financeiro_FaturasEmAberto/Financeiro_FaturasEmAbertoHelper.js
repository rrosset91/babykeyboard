({
      ...
      if (state === "SUCCESS") {
        if (data.success) {
          data.invoices.forEach((x) => {
            ...
            x.hasContestation =
              x.contestation != null && x.contestation != undefined;
    ...
    action.setCallback(this, function (response) {
      ...
      if (state === "SUCCESS") {
        if (data.success) {
          ...
          let realDate = JSON.parse(
            JSON.stringify(data.invoiceDetails.dataVencimento)
          );
          realDate = realDate.split("/");
          ...
          allInvoices.forEach((x) => {
            if (data.invoiceDetails.idFatura == x.idFatura)
              data.invoiceDetails.contestation = x.contestation;
          });
  ...
  postInvoiceDuplicates: function (component, invoice, callback) {
    ...
    console.log(
      ...
      JSON.parse(JSON.stringify(component.get("v.selectedSendMethod")))
    );
