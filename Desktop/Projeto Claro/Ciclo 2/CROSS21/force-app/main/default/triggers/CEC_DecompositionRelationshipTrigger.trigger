trigger CEC_DecompositionRelationshipTrigger on vlocity_cmt__DecompositionRelationship__c (before insert) {
    if (CEC_CatalogUtilities.isRunningInSandbox() || test.isRunningTest()){
        ...
        for(vlocity_cmt__DecompositionRelationship__c  pm : Trigger.new){
            if(pm.Chave__c == null){
