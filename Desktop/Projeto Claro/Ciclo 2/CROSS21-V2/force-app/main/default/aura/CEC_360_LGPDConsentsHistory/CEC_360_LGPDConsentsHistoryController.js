                        ...
                        for (var record in sample[data].agreements) {
                            ...
                            if(registro.name != null){
                                ...
                                if( sample[data].notes != null ){ 
                                    var reasonStr = JSON.parse(sample[data].notes).Reason;
                                    ...
                                    registro.channel = reason[1];
                                      ...
                                      registro.protocol = JSON.parse(sample[data].notes).protocolo;
                                      registro.channel = JSON.parse(sample[data].notes).sistema;
                                ...
                                if(sample[data].consentCreationDate != null){                                 
								  /*  registro.originalDate = new Date(sample[data].consentCreationDateStr);
									var diffUTC =registro.originalDate.getUTCHours() - registro.originalDate.getHours();
                                  //  console.log('diffUTC', diffUTC); 
                                   // registro.timestamp = registro.originalDate.setHours(registro.originalDate.getHours() - diffUTC);
                                   registro.timestamp = registro.originalDate.setHours(registro.originalDate.getHours() - diffUTC);
                                   //registro.timestamp = registro.originalDate.setHours(registro.originalDate.getHours());
                                    registro.consentCreationDate = registro.originalDate.toLocaleString();
                                   //registro.consentCreationDate = sample[data].consentCreationDate; */
                                   //
