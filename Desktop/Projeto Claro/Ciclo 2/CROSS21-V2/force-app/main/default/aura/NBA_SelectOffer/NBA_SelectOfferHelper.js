({
	...
	endCaseSuspension: function (component, event, helper) {
        component.set('v.showSpinner', true);
        ...
        var lstIsencoes = component.get('v.lstIsencoes');
        var subscriberId = component.get('v.subscriberId');	
        var action = component.get('c.endCaseSuspension');
        action.setParams({
            "recordId" : caseId,
            "oferta" : JSON.stringify(ofertaSelecionada),
            "produtoAtual" : JSON.stringify(produtoAtual),
	...
	encerrarCaso: function(component) {
        ...
        action.setParams({
			...
			"sistemaIndisponivel"	: component.get('v.hasAnaliseError')
        });
	...
	varrerProdutos: function(component) {
		...
		if (oferta.movimentBroadband === 'Aquisição' && oferta.broadband.planBroadband.fidelity.hasFidelity === true) {
			hasRenovation.broadband = true;                        
            component.set('v.months',oferta.broadband.planBroadband.fidelity.loyaltMonths);
		} else {
			hasRenovation.broadband = false;
		}
    
		if( oferta.movimentTv === 'Aquisição' && oferta.tv[oferta.techSelected].planTv.fidelity.hasFidelity === true){
			hasRenovation.tv = true;
            component.set('v.months',oferta.tv[0].planTv.fidelity.loyaltMonths);
		} else {
			hasRenovation.tv = false;
		}
    
		if( oferta.movimentPhone === 'Aquisição' && oferta.phone.planPhone.fidelity.hasFidelity === true) {
			hasRenovation.phone = true;
            component.set('v.months',oferta.phone.planPhone.fidelity.loyaltMonths);
		} else {
			hasRenovation.phone = false;
		} 
		if(oferta.mobile.planMobile.fidelity.hasFidelity === true){
			hasRenovation.mobile = true;
			component.set('v.months',oferta.mobile.planMobile.fidelity.loyaltMonths);
		}else {
			hasRenovation.mobile = false;
		}
		...
		if(hasRenovation.mobile){
			productsRenovation += 'MB';
		}
		...
		if ((oferta.movimentPhone != 'Cancelamento' && oferta.phone.planPhone.label != 'NÃO POSSUI') ||
		    (oferta.movimentMobile != 'Cancelamento' && oferta.mobile.planMobile.label != 'NÃO POSSUI')) {
			component.set('v.showOperatorMessage', true);
		}

		if (oferta.movimentMobile == 'Aquisição') {
			component.set('v.showChipMessage', true);
		}
            ...
            if (response.getState()=="SUCCESS"){
				...
				if(data.validationNeeded){
					if(component.get("v.subscriberId")=='')
						component.set('v.OfferValidationMessage', {message:$A.get("$Label.c.NBA_OfferValidationMissingInfo"),tabulations:['IMPRODUTIVO','VALIDAÇÃO OFERTA','SISTEMA INDISPONÍVEL'],showModal:true,showReturn:false});
					else if(!data.ValidationSuccess)
						component.set('v.OfferValidationMessage', {message:$A.get("$Label.c.NBA_OfferValidationInternalError"),tabulations:['IMPRODUTIVO','VALIDAÇÃO OFERTA','SISTEMA INDISPONÍVEL'],showModal:true,showReturn:true});
					else if(data.statusCode==200){
						component.set('v.OfferValidationScheduling', data.response.data.scheduling);
						component.set('v.OfferValidationRequest', data.request);
						$A.enqueueAction(component.get('c.incluirModal'));
					}
					else if(data.response.error.textMessages.length>0 && data.statusCode!=500){
						var formatedTextMessage = data.response.error.textMessages.join(', ');
						formatedTextMessage = formatedTextMessage.charAt(0).toUpperCase() + formatedTextMessage.slice(1);
						component.set('v.OfferValidationMessage', {message:formatedTextMessage,tabulations:['IMPRODUTIVO','VALIDAÇÃO OFERTA','PENDÊNCIA SISTÊMICA'],showModal:true,showReturn:true});
					}
					else
						component.set('v.OfferValidationMessage', {message:$A.get("$Label.c.NBA_OfferValidationAPIErrorWOMsg"),tabulations:['IMPRODUTIVO','VALIDAÇÃO OFERTA','SISTEMA INDISPONÍVEL'],showModal:true,showReturn:true});
				}
				else{
            ...
            action.setCallback(this, function(response){
                ...
                if (state === "SUCCESS") {
                    ...
                    if (data.data.schedule === null || data.data.schedule.length === 0){
						...
						component.set('v.semDatasAgendamento', false);
    ...
    dateFidelity: function(component){
        var hasRenovation = component.get('v.hasRenovation');
        var offerMonth;   
        
        if(hasRenovation.tv){
         offerMonth = component.get('v.ofertaSelecionada.tv[0].planTv.fidelity.loyaltMonths');
        }
        else if(hasRenovation.broadband){
         offerMonth = component.get('v.ofertaSelecionada.broadband.planBroadband.fidelity.loyaltMonths');
        }
        else if(hasRenovation.phone){
         offerMonth = component.get('v.ofertaSelecionada.phone.planPhone.fidelity.loyaltMonths');
		}
		else if(hasRenovation.mobile){
		 offerMonth = component.get('v.ofertaSelecionada.mobile.planMobile.fidelity.loyaltMonths');
		}        

        let tvValue = component.get("v.ofertaSelecionada.tv[0].planTv.fidelity.totalPenaltyValue") / offerMonth;
        let broadBandValue = component.get("v.ofertaSelecionada.broadband.planBroadband.fidelity.totalPenaltyValue") / offerMonth;
		let phoneValue = component.get("v.ofertaSelecionada.phone.planPhone.fidelity.totalPenaltyValue") / offerMonth;
		let mobileValue = component.get("v.ofertaSelecionada.mobile.planMobile.fidelity.totalPenaltyValue") / offerMonth;		
		//converted values pt-BR
		 let tvValueBR = (tvValue.toFixed(2)).toLocaleString('pt-BR', { minimumFractionDigits: 2});
		 let broadBandValueBR = (broadBandValue.toFixed(2)).toLocaleString('pt-BR', { minimumFractionDigits: 2});
		 let phoneValueBR = (phoneValue.toFixed(2)).toLocaleString('pt-BR', { minimumFractionDigits: 2});
		 let mobileValueBR = (mobileValue.toFixed(2)).toLocaleString('pt-BR', { minimumFractionDigits: 2});

        component.set('v.installmentsTV',offerMonth +'X '+'R$'+tvValueBR);
        component.set('v.installmentsBL',offerMonth +'X '+'R$'+broadBandValueBR);
		component.set('v.installmentsPH',offerMonth +'X '+'R$'+phoneValueBR);
		component.set('v.installmentsMB',offerMonth +'X '+'R$'+mobileValueBR);
        var result = new Date();
        result.setMonth(result.getMonth() + offerMonth);		
		var today = $A.localizationService.formatDate(result, "dd/MM/yyyy");
		component.set('v.today', today);        

    },

	hasGestorCredito : function (component, event, helper){
		var sPageURL			= ' ' + window.location;
        var sURL				= sPageURL.split('/');
		var caseId				= sURL[sURL.length - 2];
		var cityCode            = component.get('v.cityCode');
			...
			if (state === "SUCCESS") {
				...
				if(!dataResult.analiseError){
					if(dataResult.creditSituation.status != "APROVADO"){
						...
						if (runAgendamento) {
							this.hasAgendamento(component, event);
						} else {
							component.set('v.showSpinner', false);
							component.set('v.ShowModal', true);
							component.set('v.showContent', true);
