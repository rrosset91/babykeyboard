({
    doInit : function(component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            var concents    = response.getReturnValue()['getConsents']; 
            var putConsents = response.getReturnValue()['putConsents'];  

            if( state === "SUCCESS") {
                
                //PROBABLY WE NEED TO CHANGE purposes[0] FOR SPECIFIC purpose FIND BY Id
                if( concents.purposes != null && concents.purposes[0].agreements != null ) { 
                   
                    list = concents.purposes[0].agreements;  
 
                    // assign checked in consent node level according to OneTrust
                    for(var key in list){

                        if( putConsents.purposes[0].agreements[key].options.length > 2 )
                            putConsents.purposes[0].agreements[key].options.splice(0, 2); 
                        else
                            putConsents.purposes[0].agreements[key].options.splice(0, 1); 
                    ...
                    console.log(' putConsents => ', putConsents );
                     
                    component.set('v.consents',    list); 
                    component.set('v.putConsents', putConsents );
                    ...
                    if(concents.error){
                        console.log("ERRO: " + JSON.stringify(concents.error));
                    }
                }
            } else if (state === "ERROR") {

                component.set('v.consents', null);
                var errors = concents.getError();
    ...
    handleSaveButton: function(component, event, helper) {
        ...
        var consents    = component.get("v.consents");
        var putConsents = component.get("v.putConsents");
        ...
        actionGetProtocol.setCallback(this, function (response) {
            ...
            console.log('Protocolo => ', response.getReturnValue());
            ...
            if(state === "SUCCESS") {
                ...
                if( protocolNumber )
                    putConsents[0]['protocolNumber'] = protocolNumber; 
                ...
                for(var key in consents){ 
                    ...
                    if(consents[key].checked){
                        putConsents[0].purposes[0].agreements[key].options[0]['id'] = yesId; 
                    }else{
                        putConsents[0].purposes[0].agreements[key].options[0]['id'] = noId;
                    }  
                }  
                ...
                action.setParams({
                    recordId: component.get("v.recordId"),
                    consents: JSON.stringify( putConsents[0] )
                });
                action.setCallback(this, function (response) {
                    var state = response.getState();
