({
    ...
    */
	...
	handlePageEvent: function (component, event, helper) { // evento: NBA_PageEvent
		let action = event.getParam('action');
        let data = event.getParam('data');
        
		if(data!=null && data.isNotCurrentProduct!=null)
        component.set('v.noCurrentProduct', data.isNotCurrentProduct == true ? data.isNotCurrentProduct : false);
        
		if (action == 'RESET_ORDER') {
			helper.resetOrder(component, event);
		} else if (action == 'RESET_ADDITIONALS') {
			helper.cleanAdditionals(component, event,data);
		}
	},

	handleChangeOfertaSelecionada: function(component, event, helper) {
		helper.updateTemporarySuspension(component);
	},

	updateOfferDiffsFromPossession: function(component, event, helper){
		if(component.get('v.previousPossessionalue')!=0 && component.get('v.produtoAtual.totalValue')!=null && component.get('v.previousPossessionalue')!=component.get('v.produtoAtual.totalValue')){
			console.log('updateOfferDiffsFromPossession');
			if(component.find("modalAditionalsTV")!=null)
				component.find("modalAditionalsTV").callUpdateOfferDiffs(); 
			if(component.find("modalAditionalsBroadband")!=null)
				component.find("modalAditionalsBroadband").callUpdateOfferDiffs(); 
			if(component.find("modalAditionalsPhone")!=null)
				component.find("modalAditionalsPhone").callUpdateOfferDiffs(); 
			if(component.find("modalAditionalsMobile")!=null)
				component.find("modalAditionalsMobile").callUpdateOfferDiffs(); 
		}
		if(component.get('v.produtoAtual.totalValue')!=null)
			component.set('v.previousPossessionalue', component.get('v.produtoAtual.totalValue'));
