({
    ...
    validarTelefoneContaContato: function(retornoDadosConta) {
        var dadosConta = true;
        var dadosContato = false;
        var state = retornoDadosConta.getState();
        if(state === "SUCCESS") {
            if(retornoDadosConta.getReturnValue().CellPhone__c == undefined && retornoDadosConta.getReturnValue().Phone == undefined){
                dadosConta = false;
                console.log('dados de telefone da conta vazios');
            }

            if(!dadosConta){
                if(retornoDadosConta.getReturnValue().Contacts != null){
                    for(var i=0;i< retornoDadosConta.getReturnValue().Contacts.length;i++){
                        if(retornoDadosConta.getReturnValue().Contacts[i].Phone != undefined){
                            dadosContato = true;
                            break;
                        }
                        if(retornoDadosConta.getReturnValue().Contacts[i].MobilePhone != undefined){
                            dadosContato = true;
                            break;
                        }                        
                    }
            ...
            if(!dadosContato && !dadosConta){
                var title = 'Ocorreu um erro ao verificar dados da conta.';
                var type = 'error';
                var message = 'Validar os campos de telefone da conta ou do contato.';
                
                this.showToast(title,type,message);
            }
        }

        return dadosContato || dadosConta;
    },
