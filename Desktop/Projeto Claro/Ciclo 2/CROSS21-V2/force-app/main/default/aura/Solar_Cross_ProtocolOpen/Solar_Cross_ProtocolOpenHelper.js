({
    ...
    firstCallResolution : function(component, event, helper) {
        ...
        for(var i = 0; i < leaveOptions.length; i++){
            if(leaveOptions[i].value == leaveValue){
                ...
                console.log('FCR hasPermissionOfferCallCenter[i] ' + leaveOptions[i].hasPermission);
                if(leaveOptions[i].fcr == true){
                    component.set("v.showFCR", true);
                    component.set('v.hasPermissionOfferCallCenter',leaveOptions[i].hasPermission);
                    if(leaveOptions[i].offer == true){
    ...
    updateCase : function(component, event, helper) {
        ...
        var aRadioOfferValue = component.get("v.radioOfferValue"); 
        aRadioOfferValue = aRadioOfferValue == '' || undefined || null ? null : aRadioOfferValue;

        var action = component.get("c.updaContextRecord");        
        action.setParams({aCaseId : aCaseId, aResolution : aResolution, aLeavingReason : aLeavingReason, aCategory : aCategory, /*aProductValue : aProductValue,*/ aModality : aModality, aEntry : aEntry, aDescription : aDescription, aRadioOfferValue : aRadioOfferValue});        
        action.setCallback(this, function(response) {
