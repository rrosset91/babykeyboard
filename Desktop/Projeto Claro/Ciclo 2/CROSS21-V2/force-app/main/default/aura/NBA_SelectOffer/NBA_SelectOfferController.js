({
	...
	incluirModal: function (component, event, helper) {
		...
		let hasSuspension = component.get('v.temporarySuspension.hasSuspension');
		...
		if(!componentSelectOffer && (
			selectedOffer.movimentMobile=='Aquisição' || (
				!produtoAntigo.mobile.isComboMulti && (
		...
		else if(componentSelectOffer || produtoAtual){
			if(JSON.stringify(produtoAntigo) === JSON.stringify(produtoAtual) ){
				component.set('v.isProdutoAntigo', true);
			}
			component.set('v.isActualProduct', JSON.stringify(selectedOffer) === JSON.stringify(produtoAntigo));
			...
			if(component.get('v.OfferValidationScheduling') && !hasSuspension) {
		...
		for(var i=0 ; i<produtoAntigo.mobile.listaDependentesMobilePosse.length ; i++){
			for(var j=0 ; j<produtoAtual.mobile.listaDependentesMobilePosse.length ; j++){
				if(produtoAntigo.mobile.listaDependentesMobilePosse[i].numeroTelefone == produtoAtual.mobile.listaDependentesMobilePosse[j].numeroTelefone){
					produtoAntigo.mobile.listaDependentesMobilePosse[i].checked = produtoAtual.mobile.listaDependentesMobilePosse[j].checked;
				}
			}
		}
		...
		//Tem análise de crédito
		} else if (!hasSuspension && selectedOffer.movimentBroadband !== 'Cancelamento' && selectedOffer.movimentPhone !== 'Cancelamento' && selectedOffer.movimentTv !== 'Cancelamento' && selectedOffer.movimentMobile !== 'Cancelamento' &&
				   ...
				   JSON.stringify(produtoAtual) !== JSON.stringify(selectedOffer)){
...
salvar : function(component, event, helper){
	...
	if (component.get('v.temporarySuspension.hasSuspension')) {
		helper.endCaseSuspension(component, event, helper);
		component.set('v.confirmed', true);
		component.set('v.ShowModal', false);
		return null;
	} else if(hasAgendamento && dataAgendamento && periodo){
