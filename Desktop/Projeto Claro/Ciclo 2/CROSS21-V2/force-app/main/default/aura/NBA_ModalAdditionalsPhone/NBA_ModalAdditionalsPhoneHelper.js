	...
	calculoPercentualRecalculate: function (component, event, helper, lstPontuacao, ofertaSelecionada, produtoAntigo) {
		...
		calculateOriginalValue : function(component, currentProduct){
			...
			if(currentProduct.mobile.planMobile.label != 'NÃO POSSUI'){
				var descontoBL = parseFloat(currentProduct.mobile.descontoDependenteBL);
				var descontoPG = parseFloat(currentProduct.mobile.descontoDependentePG);
				var descontoTitular = parseFloat(currentProduct.mobile.descontoTitular);
			}
			var originalValue = 0;
			var indexMaior = 0;			
		...
		calculateOriginalValueOferta : function(component, ofertaSelecionada){
			...
			var descontoTit		   = 0;
			var indexMaior 		   = 0;
			var listDepProduto     = component.get('v.produtoAtual.mobile.listaDependentesMobilePosse');
