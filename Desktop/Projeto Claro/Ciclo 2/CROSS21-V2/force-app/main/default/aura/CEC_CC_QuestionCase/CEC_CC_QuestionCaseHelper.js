 ...
 * 1.0   01-18-2021   ChangeMeIn@UserSettingsUnder.SFDoc   Initial Version
**/
({
    ...
    createRecord: function (component) {
        ...
        var recTypeTaskMap = component.get("v.recTypeTaskMap").Canais_Criticos;
        var vCase = component.get("v.parentCase");
        var telefone1 = '';
        var telefone2 = '';
        var contactId = vCase && vCase.ContactId ? vCase.ContactId : null;

        if (vCase.Account.Phone != null && vCase.Account.CellPhone__c != null){
            telefone1 = vCase.Account.Phone;
            telefone2 = vCase.Account.CellPhone__c;
        } else if(vCase.Account.Phone != null){
            telefone1 = vCase.Account.Phone;
        } else if (vCase.Account.CellPhone__c != null){
            telefone1 = vCase.Account.CellPhone__c;
        }

        var evt = $A.get("e.force:createRecord");

        //Se nao tiver o contato não pode enviar o elemento WhoId, nem mesmo com '', porque da erro
        
        var params = {
            'entityApiName':'Task',
            'defaultFieldValues': {
                'WhatId': vCase.Id,
                'Telefone1__c': telefone1,
                'Telefone2__c': telefone2
            },
            'recordTypeId': recTypeTaskMap
        };
        
        if(contactId) {
            
            params.defaultFieldValues.WhoId = contactId;
    ...
    getRecordTypeTask : function(component)
    {
        var action 	= component.get("c.getRecordTypeDetail");   
        action.setParams({ varObj: 'Task'});
        action.setCallback(this, function(response) {	
            if(response.getState() === "SUCCESS") { 
                component.set("v.recTypeTaskMap", response.getReturnValue());
            }
            else if(response.getState() === "ERROR" || response.getState() === "INCOMPLETE") {
                var errors = response.getError();
                if (errors[0] && errors[0].message) 
