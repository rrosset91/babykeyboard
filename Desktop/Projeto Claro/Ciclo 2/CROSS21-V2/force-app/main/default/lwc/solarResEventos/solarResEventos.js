...
export default class SolarResEventos extends LightningElement {
	...
	@track
	accordionFunctions = {
		comprasCancelamentosByDate: getEventsByDate,
		comprasCancelamentosByPeriod: getEvents,

		comprasMovimentacoesByDate: getEventsByDate,
		comprasMovimentacoesByPeriod: getEvents,

		eventosTecnicosByDate: getEventsByDate,
	...
	eventAccordions = [
		"comprasCancelamentosByDate",
		"comprasCancelamentosByPeriod",
		"comprasMovimentacoesByDate",
		"comprasMovimentacoesByPeriod",
		"eventosTecnicosByDate",
		"eventosTecnicosByPeriod"
	];
	...
	columns = {
		general: [
			{
				label: "Descrição",
				fieldName: "requestTypeDescription",
				type: "text",
				sortable: true,
		...
		history: [
			{
				label: "Descrição",
				fieldName: "extractItemTypeDescription",
				type: "text",
				sortable: true,
				hideDefaultActions: true
			},
	...
	handleFilterNotification(event) {
		if (event.detail.filterType === "CUSTOMPERIOD") {
			...
			if (!Utils.isEmptyString(String(this.startDate)) && !Utils.isEmptyString(String(this.endDate))) {
			...
			if (!Utils.isEmptyString(String(this.period))) {
	...
	handleOpenModal(relatedAccordion) {
		let specialAccordion =
			relatedAccordion.includes("comprasCancelamentos") || relatedAccordion.includes("comprasMovimentacoes") || relatedAccordion.includes("eventosTecnicos");
	...
	async fetchEvent(newAccordion) {
		...
		let newAccordionFunction;
		if (this.filterType === "custom") {
			console.log("BYDATE===>", newAccordion);
			newAccordionFunction = newAccordion + "ByDate";
		} else {
			console.log("BYPERIOD===>", newAccordion);
			newAccordionFunction = newAccordion + "ByPeriod";
		}
	...
	handleSort(context, event, data) {
		...
		if (sortField == "formattedDate") sortField = "realDate";
	...
	handleSortOcorrenciasAcompanhamentos = (evt) =>
		(this.eventsValues.ocorrenciasAcompanhamentos = this.handleSort("ocorrenciasAcompanhamentos", evt, this.eventsValues.ocorrenciasAcompanhamentos));

	handleSortOcorrenciasFinanceiras = (evt) =>
		(this.eventsValues.ocorrenciasFinanceiras = this.handleSort("ocorrenciasFinanceiras", evt, this.eventsValues.ocorrenciasFinanceiras));

	handleSortOcorrenciasCancelamento = (evt) =>
		(this.eventsValues.ocorrenciasCancelamento = this.handleSort("ocorrenciasCancelamento", evt, this.eventsValues.ocorrenciasCancelamento));
